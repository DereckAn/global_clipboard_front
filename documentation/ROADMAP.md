...
### 1.4 Icono del tray adaptable ✅

- Icono monocromático (`tray_icon_template.png`) embebido en el binario y marcado como template; macOS invierte automáticamente según fondo.
- Se fuerza `show()+focus()+always_on_top toggle` para que la ventana principal aparezca al frente al arrancar.

### 1.5 Files `.log` con preview textual ✅

- `.log` reconocido como `text/plain` para generar snippets.

### 1.6 Poner linter y formatear el código en CI/CD

- [ ] Configurar pipeline (GitHub Actions) que ejecute `cargo fmt --check`, `cargo clippy`, `bun run check`.

### 1.7 Preview en otros OS (Windows y Linux) ✅

- Windows: `IShellItemImageFactory` genera thumbnails exactos.
- Linux: se aprovecha `gdk-pixbuf-thumbnailer` + fallbacks (PDF/texto). Se recomienda tener `gdk-pixbuf` y `poppler` instalados.

### 1.8 Dropdown filter global

- [ ] La barra de filtros debe ejecutar búsqueda global y luego paginar resultados (hoy el filtrado es local sobre items cargados). Ajustar `clipboardStore.search()` y el dropdown para disparar queries al backend.

### 1.9 Integración LAN / QR

- [ ] Estudiar broadcast sobre LAN (mDNS o WebRTC local) para enviar archivos/links entre dispositivos, mostrando QR para conexión rápida.

### 1.10 Clipboard para terminal

- [ ] CLI para pegar contenido en terminal (por ejemplo `clipctl select <id> | pbcopy`). Rocket/Tauri plugin? Evaluar watchers en shell.
