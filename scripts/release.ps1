...
if ($cargoContent -match 'version\s*=\s*"([^"]+)"') {
    ...
    Write-Red "❌ No se pudo leer la versión de Cargo.toml"
    exit 1
}

Write-Host "📦 Versión actual en Cargo.toml: " -NoNewline -ForegroundColor Cyan
...
if ($gitStatus) {
    Write-Yellow "⚠️  Hay cambios sin commitear:"
    Write-Host ""
    git status -s
    Write-Host ""
    ...
    if ($reply -match '^[Yy]$') {
        ...
        if ([string]::IsNullOrWhiteSpace($COMMIT_MSG)) {
            Write-Red "❌ Error: El mensaje del commit no puede estar vacío"
            exit 1
        }
        ...
        Write-Yellow "⚠️  Continuando sin hacer commit de los cambios..."
        Write-Host ""
    }
} else {
...
if ($appTagReply -match '^[Yy]$') {
    ...
    if ($existingTag) {
        Write-Yellow "⚠️  El tag $APP_TAG ya existe"
    ...
    if ($APP_TAG) {
        Write-Green "🏷️  Creando tag $APP_TAG"
...
if ($helperReply -match '^[Yy]$') {
    # Sugerir versión
    Write-Host ""
    $HELPER_VERSION = Read-Host "Versión para helper (ej: 1.0.0)"
    
    if ([string]::IsNullOrWhiteSpace($HELPER_VERSION)) {
        Write-Red "❌ Error: La versión no puede estar vacía"
    } else {
        ...
        if ($existingHelperTag) {
            Write-Yellow "⚠️  El tag $HELPER_TAG ya existe"
        ...
        if ($HELPER_TAG) {
            Write-Green "🏷️  Creando tag $HELPER_TAG"
