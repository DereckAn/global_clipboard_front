...
bun run dev                   # start Vite dev server for the web UI
bun run tauri dev             # launch desktop build with Tauri CLI
bun run build                 # create production frontend bundle
bun run check                 # sync SvelteKit and run type analysis
bun run test                  # execute all Rust tests
bun run test:unit             # backend unit tests only
bun run test:integration      # cleanup integration suite
bun run test:all              # type check + Rust tests in one go
```
- Run commands from the repo root; regenerate locks with `bun install` or `cargo update` only when necessary.

## Coding Style & Naming Conventions
- Use TypeScript with 2-space indentation; components stay in `PascalCase.svelte`, stores and util modules export `camelCase` APIs, and keep derived helpers near their stores.
- Rust files remain `snake_case` and must pass `cargo fmt`; prefer `anyhow::Result` for shared error handling and log via `tracing` or `println!` consistently.
- Before pushing, run `bun run check` and `cargo fmt && cargo clippy --fix --allow-dirty` to keep both toolchains lint-clean.
...
- PR descriptions should outline user-visible impact, link tracking issues, and attach screenshots or clips for UI updates.
- Call out which commands you ran and highlight any follow-up tasks or configuration changes reviewers should apply locally.
