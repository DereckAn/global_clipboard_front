<script lang="ts">
    ...
    const options: Array<{
      ...
      label: string
      icon: keyof typeof
  import('$lib/components/icons/icons.svelte').iconPaths
    }> = [
  ...
  <div class="relative" data-filter-dropdown>
    ...
    <button
      ...
      class="flex items-center gap-2 px-3 py-2 rounded-md border border-border bg-surface hover:bg-surface-hover transition-colors text-sm"
    >
